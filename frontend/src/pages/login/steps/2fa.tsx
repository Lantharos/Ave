...
export default function MFALogin({handler, setData, data}: Props) {
    function renderMethods() {
        const methods = [
            {
                value: "Continue with Authenticator",
                key: "totp"
            },
            {
        ...
        return methods.map((method, idx) => {
            ...
            return (
                <div
                    ...
                    onClick={() => {
                        setData({...data, selectedMFAMethod: method.key});
                        handler(5);
                    }}
                >
    ...
    return (
        <>
            ...
            <div className={"flex flex-col items-start justify-end gap-[25px] ml-[10px] w-full h-full"}>
                <h1 className={"text-[24px] font-[500]"}>We need a bit more</h1>
