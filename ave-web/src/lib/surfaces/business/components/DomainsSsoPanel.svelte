  ...
  <div class="grid gap-5 xl:grid-cols-[0.9fr_1.1fr]">
        ...
        <div class="grid gap-3">
          {#each detail.domains as domain (domain.id)}
            <div class="rounded-[14px] bg-[#151515] px-4 py-4">
              ...
              {#if domain.status !== "verified"}
                <p class="m-0 mt-3 break-all rounded-[12px] bg-[#0d0d0d] px-4 py-3 text-[12px] text-[#9a9a9a]">_ave-challenge.{domain.domain} TXT {domain.token}</p>
              {/if}
            </div>
          {/each}
        </div>
    ...
    <Panel>
      <div class="flex flex-col gap-5">
        ...
        <div class="flex flex-wrap items-center justify-between gap-4 rounded-[14px] bg-[#151515] px-4 py-4">
          <div class="min-w-0">
        ...
        <div class="grid gap-3">
          ...
          {#if !detail.ssoConnections.length}
            <div class="rounded-[14px] bg-[#151515] px-4 py-5 text-[14px] text-[#777]">No SSO connections yet.</div>
          {/if}
        </div>
      </div>
    </Panel>
