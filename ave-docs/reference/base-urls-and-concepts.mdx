...
| Host | Used for |
|---|---|
| `aveid.net` | User-facing pages (sign-in, Connector consent) |
| `api.aveid.net` | All API calls (token exchange, userinfo, JWKS, signing) |

```
✅ POST https://api.aveid.net/api/oauth/token
❌ POST https://aveid.net/api/oauth/token   ← wrong host
```
...
<AccordionGroup>
  ...
  <Accordion title="access_token_jwt — signed JWT for your backend">
    A signed JWT with `aud: https://api.aveid.net`. Use this when calling Ave API endpoints that accept JWTs, or when your own backend wants to verify the token locally.

    ```
    access_token_jwt: "eyJ..."
    ```

    Verify using the `jwks_uri` from your issuer's OIDC discovery document (`{issuer}/.well-known/openid-configuration`). For the default Ave issuer this is `https://api.aveid.net/.well-known/jwks.json`.
  </Accordion>
...
<Note>
  `id_token` and `access_token_jwt` are both JWTs but have **different audiences**. The `id_token` audience is your `clientId`. The `access_token_jwt` audience is `https://api.aveid.net`. This matters for any library that validates JWT audience.
</Note>
