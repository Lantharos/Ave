...
- Convex `domain` config — stays `https://api.aveid.net`
...
<Steps>
  ...
  <Step title="Replace handleQuickCallback with exchangeCode">
    ```ts
    ...
    <Note>
      `startPkceLogin` does not generate or store a `state` value. If you need `state`-based CSRF protection, use `buildAuthorizeUrl` directly, generate a state with `crypto.randomUUID()`, store it yourself, and verify it here.
    </Note>

    Unlike `handleQuickCallback`, `exchangeCode` returns the full `TokenResponse` — you manage your own session from here.
  </Step>
