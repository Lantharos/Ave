...
SQLite / dialect differences: use your driver’s upsert form, or **`onConflictDoNothing`** then **`findFirst`** by **`aveIdentityId`**, or **try/catch** on unique constraint and **`findFirst`** on retry.
