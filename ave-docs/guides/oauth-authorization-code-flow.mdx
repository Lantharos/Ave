...
<Steps>
  ...
  <Step title="Validate id_token and create session">
    For security-critical systems, validate the `id_token` before trusting it. Use `verifyJwt` from the SDK — it handles JWKS fetching, RS256 signature verification, and all required claim checks automatically.
    ...
    const claims = await verifyJwt(tokens.id_token, {
      ...
      nonce: savedNonce, // the nonce you stored before redirecting
    });
    ...
    <Tip>
      `verifyJwt` checks signature, issuer (`https://aveid.net`), audience, expiry, and nonce in one call. It caches the JWKS for 5 minutes. For manual validation or non-JS environments, see the validation sequence below.
    </Tip>
  </Step>
</Steps>
