...
<Note>
  If you are building Convex auth, use `id_token`. Its `aud` is your `clientId`, which is what Convex validates against. The `access_token_jwt` has `aud: "https://api.aveid.net"` — Convex will reject it because the audience does not match your app.
</Note>
...
|---|---|---|
| `iss` | Both JWTs | Always `https://api.aveid.net` |
| `sub` | Both JWTs | Identity UUID (changes per Ave identity) |
| `aud` | Both JWTs | `clientId` in `id_token`; `https://api.aveid.net` in `access_token_jwt` |
...
<Steps>
  <Step title="Create request context">
    Generate `state`, `nonce`, and PKCE parameters before redirecting. Store them temporarily in `sessionStorage` (browser) or your server session.
  ...
  <Step title="Handle callback securely">
    Ave redirects to your `redirect_uri` with `?code=...&state=...`.
    ...
    // Clear immediately — they are one-time use
  ...
  <Step title="Validate id_token and create session">
    For security-critical systems, validate the `id_token` before trusting it.

    ```ts
    // Fetch JWKS (cache this — it rarely changes)
    const { keys } = await fetch("https://api.aveid.net/.well-known/jwks.json").then(r => r.json());

    // Validate using your JWT library (jose, jsonwebtoken, etc.)
    // Required checks:
    // 1. Signature valid against JWKS
    // 2. iss === "https://api.aveid.net"
    // 3. aud === YOUR_CLIENT_ID
    // 4. exp > Date.now() / 1000
    // 5. nonce === savedNonce (if you sent one)
    ```

    <Tip>
      Libraries like `jose` handle JWKS fetching and validation automatically. Pass `issuer`, `audience` (your client ID), and `clockTolerance` options.
    </Tip>
  </Step>
</Steps>
...
<Steps>
  <Step title="Fetch JWKS">
    `GET https://api.aveid.net/.well-known/jwks.json`. Cache the response and re-fetch only when you encounter an unknown `kid`.
  </Step>
  ...
  <Step title="Verify issuer and audience">
    `iss` must equal `https://api.aveid.net`. `aud` must equal your client ID. Reject anything else.
  </Step>
...
- Authorization codes expire in seconds — do not store and exchange later
- Redirect URI must match exactly, including protocol and path
- Missing `openid` scope means no `id_token` is returned
- Missing `offline_access` scope means no `refresh_token` is returned
- If your app requests `user_id` but does not have `allowUserIdScope` configured, the `uid` claim will not appear
