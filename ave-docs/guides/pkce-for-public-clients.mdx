...
<Check>Store `code_verifier`, `state`, and `nonce` in `sessionStorage` (not `localStorage`)</Check>
<Check>Verify the returned `state` on the callback before exchanging the code — prevents CSRF attacks</Check>
