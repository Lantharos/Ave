...
<Steps>
  ...
  <Step title="Key handoff via URL fragment">
    After a successful authorization, Ave passes `encrypted_app_key` to your callback as a URL fragment (`#encrypted_app_key=...`). It is **not** included in the JSON token response — this ensures it never appears in server logs, proxy access logs, or JSON response bodies. Parse it from the fragment before exchanging the authorization code.

    ```ts
    // Parse the encrypted key from the URL fragment before clearing it
    const hashParams = new URLSearchParams(window.location.hash.slice(1));
    const rawKey = hashParams.get("encrypted_app_key");
...
`encrypted_app_key` is always delivered as a URL fragment (`#encrypted_app_key=...`). Browsers silently replace `+` with a space in fragment parameters, which breaks base64 decoding.
