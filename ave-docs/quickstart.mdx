...
<Steps>
  ...
  <Step title="Exchange the code for tokens">
    <Tabs>
      ...
      <Tab title="Raw HTTP">
        ...
        <Warning>
          Ave uses **camelCase** fields (`grantType`, `clientId`, `redirectUri`, `codeVerifier`) — not the snake_case used by most OAuth providers. Sending `grant_type` or `client_id` returns a `400` error.
        </Warning>
      </Tab>
    </Tabs>
  </Step>
  <Step title="Understand the token response">
    ...
    | `access_token_jwt` | Signed JWT | `https://api.aveid.net` | Your own API, Connector delegation |
    ...
    <Note>
      `id_token` and `access_token_jwt` are both signed JWTs with **different audiences**. The `id_token` audience is your `clientId`. The `access_token_jwt` audience is `https://api.aveid.net`. This matters for Convex and any library that validates JWT audience.
  ...
  <Step title="Create your app session">
    ...
    // Option B: validate id_token claims for higher assurance
    // 1. Fetch JWKS from https://api.aveid.net/.well-known/jwks.json
    // 2. Verify signature, iss, aud === YOUR_CLIENT_ID, exp, nonce
    ```
