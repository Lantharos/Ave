...
<ParamField path="params.organizationId" type="string">
...
<ParamField path="params.prompt" type={`"none" | "login" | "consent" | "select_account" | string`}>
  OAuth-style interaction hint. Use `select_account` when your app offers **Switch account** or **Sign out** so Ave always shows the swipe-to-authorize screen and identity picker instead of silently reusing the last authorization. Combine values with a space (for example `consent select_account`).
</ParamField>

<ParamField path="params.extraParams" type="object">
  Extra query parameters on the Ave sign-in URL. Prefer standard OAuth parameters; encryption is requested via `e2ee:*` scopes — see [App encryption](/sdk/sdk-identity-keys).
</ParamField>
...
### `buildSwitchAccountUrl(config, params?)`

Convenience wrapper around `buildAuthorizeUrl` that sets `prompt=select_account`. Use from your app's **Switch account** or **Use a different profile** action so Ave blocks quick re-authorization and always shows the identity picker plus swipe-to-authorize UI.

```ts
function buildSwitchAccountUrl(
  config: AveConfig,
  params?: Parameters<typeof buildAuthorizeUrl>[1]
): string
```

```ts
import { buildSwitchAccountUrl, generateCodeChallenge, generateCodeVerifier } from "@ave-id/sdk";

const verifier = generateCodeVerifier();
const challenge = await generateCodeChallenge(verifier);

window.location.href = buildSwitchAccountUrl(
  { clientId: "YOUR_CLIENT_ID", redirectUri: "https://yourapp.com/callback" },
  {
    scope: ["openid", "profile", "email", "offline_access"],
    state: crypto.randomUUID(),
...
For embed flows, pass `extraParams: { prompt: "select_account" }` to `openAveSheet` / `openAvePopup` instead.
...
Set `organizationId` to sign into an Ave Business organization as an app workspace. Ave verifies membership before issuing tokens and includes organization claims in the returned JWTs. See [Business workspaces](/guides/business-workspaces).
...
- [Identity keys and wrapped payloads](/sdk/sdk-identity-keys) — `getIdentityKey`, envelope decryption, and passing secrets through redirects after OAuth.
