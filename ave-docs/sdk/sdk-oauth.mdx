...
Ave returns the normal token payload plus `encryptedAppKey` when the selected app/identity already has one stored. If the app needs a first-time consent or E2EE setup step, Ave continues in a FedCM dialog and then resumes automatically.
