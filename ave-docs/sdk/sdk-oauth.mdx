...
Stores `ave_code_verifier` and `ave_nonce` in `sessionStorage`. Read them back on your callback page before calling `exchangeCode`.

<Note>
  `startPkceLogin` does **not** generate or store a `state` value for CSRF protection. If your threat model requires state validation, use `buildAuthorizeUrl` directly, generate a `state` yourself with `crypto.randomUUID()`, store it in `sessionStorage`, include it in the authorize URL, and verify it on the callback.
</Note>
