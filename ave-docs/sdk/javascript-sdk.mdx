...
interface QuickIdentity {
   ...
   * Fine for prototypes and internal tooling. In production, always verify
   * server-side: check `iss`, `aud`, `exp`, and the JWT signature against the
   * JWKS endpoint. Upgrade to Standard Ave for confidential flows.
   */
  token: string;
  /**
