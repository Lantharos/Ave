...
Exchanges a source app `access_token_jwt` for a short-lived delegated token scoped to the target resource. Use this for PKCE connector flows (public client).
...
Saves `ave_connector_state`, `ave_code_verifier`, and `ave_nonce` to `sessionStorage`. Read them back on your callback page.
