...
Exchanges a FedCM assertion for the normal token payload. For E2EE apps, this may also return `encryptedAppKey` when Ave already has one stored for the selected app/identity.
