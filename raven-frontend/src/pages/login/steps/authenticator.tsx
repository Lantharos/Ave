...
import OtpInput from "../../../components/Basic/OtpInput.tsx";
type Props = {
    handler: React.Dispatch<React.SetStateAction<number>>;
    setData: React.Dispatch<React.SetStateAction<object>>;
};

export default function TOTPLogin({handler, setData}: Props) {
    return (
        <>
            ...
            <div className={"flex flex-col items-center gap-[50px] ml-[10px] w-full h-full"}>
                <div className={"flex flex-col items-start justify-center gap-[20px] w-full h-full"}>
                    <h1 className={"text-[24px] font-[400]"}>Log in with Authenticator</h1>
                    <OtpInput onComplete={(code) => setData((prevState) => ({...prevState, totp: code}))} />
                </div>
            </div>
        </>
    )
