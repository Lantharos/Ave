...
<div class="fixed inset-0 z-50">
  ...
  <div class="relative z-10 flex min-h-full items-center justify-center px-3 py-6">
    <div
      class="max-h-[calc(100vh-48px)] w-full max-w-[980px] overflow-y-auto rounded-[30px] bg-[#111111]/95 p-5 shadow-[0_32px_100px_rgba(0,0,0,0.42)] md:p-6"
      role="dialog"
      aria-modal="true"
    ...
    >
      <div class="flex flex-wrap items-start justify-between gap-4">
        ...
        <div class="flex items-center gap-3">
          ...
          <button
            class="flex min-h-10 w-10 items-center justify-center rounded-full bg-white/[0.04] text-[#8c8c8c] transition-[background-color,color,scale] duration-300 hover:bg-white/[0.07] hover:text-white active:scale-[0.96]"
            onclick={onClose}
            disabled={busy}
            aria-label="Close"
          >
      ...
      {:else}
        <div class="mt-3 grid gap-3">
          ...
          <textarea
            ...
            class="min-h-28 w-full resize-y rounded-[24px] bg-white/[0.04] px-5 py-4 text-[14px] text-white outline-none transition-colors duration-300 placeholder:text-[#555] focus:bg-white/[0.07]"
          ></textarea>
        </div>
      {/if}
